...
comet = Universe.Planets("comet", "black", 1e14, np.array((4*au,4*au)), np.array((-20000.,-15000)))
...
ss.dodaj_planet(comet)

ss.putanja(5*godina)

xs, ys, xm, ym, xv, yv, xe, ye, xm1, ym1, xc, yc = [], [], [], [], [], [], [], [], [], [], [], [] # liste za plotanje planeta
fig = plt.figure(figsize=[10, 10])
plt.rcParams["figure.autolayout"] = True
axes = fig.add_subplot(1,1,1)
ax = plt.axes(xlim=(-7.5e11, 7.5e11), ylim=(-7.5e11, 7.5e11))
line, = ax.plot([], lw = 2)
ax.set_aspect('auto')
ax.axis('off')
...
linec, = ax.plot([], [], 'o',color = comet.color)
...
def init(): # za pokretanje plotanja pridruzuje se prazna lista
    ...
    linem1.set_data([],[])
    linec.set_data([],[])
    return line, linem, linev, linee, linem1, linec
...
plt.plot(comet.x,comet.y,label=comet.ime_planeta,color=comet.color)

anim = animation.FuncAnimation(fig, animacija, init_func=init, interval=20, blit = True)
plt.title('Planeti i komet animacija graf')
